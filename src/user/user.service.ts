...
import { orderMail , orderStatusUpdateEmail} from "../constants/message.constants";
...
import { REWARD_TYPES } from "../constants/hr.constants";
...
const validateProducts = async (itemsList: any) => {
	...
	for (let item of itemsList) {
		...
		if (productObj && productObj.isActive === true) {
	...
	try {
		...
		if (!isValidList) {
			...
			if (totalPoints <= USER_POINTOBJ.available) {
				for (let product of productList) {
					...
					const order = await OrderSchema.create({
						...
						productDetails: {
							...
							productImgURL: product.productInfo.productImgURL,
						},
					...
					//Update orderId in Transaction Schema
				...
				const totalCost = ordersObj.reduce(
					(prev: any, obj: any) => prev + obj?.productDetails?.rewardPoints,
					0,
				);
				emailContent = {
					employeeId: employeeId,
				...
				const orderEmailContent = await orderStatusUpdateEmail(emailContent);
		...
		logger.error(
			"Error in User Service - Place Order - " + JSON.stringify(error?.message),
		);
		const errorObj = {
...
/**
 * @description Pushes the individual order data into userOrdersResponse Object from transactionObject
 * @param {UserOrderResponse} userOrdersResponse
 * @param  {any} order
 * @returns {UserOrderResponse}
 */
const setUserOrderFromTransactionObject = (
	ordersObj: OrdersResponseObject[],
	transactionObject: TransactionObject,
	userDetails: UserInfo,
): OrdersResponseObject[] => {
	ordersObj.push({
		orderId: transactionObject.orderId,
		quantity: transactionObject.orderDetails.quantity,
		customisation: transactionObject.orderDetails.customisation
			? { size:transactionObject.orderDetails.customisation?.size}
			: {},
		status: transactionObject.orderHistory.history[transactionObject.orderHistory.history.length - 1].status,
		productDetails: {
			productId: transactionObject.orderDetails.productDetails.productId,
			title: transactionObject.orderDetails.productDetails.title,
			rewardPoints: transactionObject.orderDetails.productDetails.rewardPoints,
			isCustomisable: transactionObject.orderDetails.productDetails.isCustomisable,
			productImgURL: transactionObject.orderDetails.productDetails.productImgURL,
		},
		orderHistory: transactionObject.orderHistory?.history,
		userDetails,
	});
	return ordersObj;
};
...
exports.getOrdersByEmployeeId = async (req: Request, res: Response) => {
	...
	// let userOrderList;
	if(orderId !== undefined && transactionId !== undefined && Number(orderId) !== 0) {
		// Get the Orders list from DB
		let orderIdNumber = Number(orderId);
		let transactionIdNumber = Number(transactionId);
		const transactionObject = await TransactionSchema.aggregate([
			{ $match: { "orderId": orderIdNumber, "transactionId": transactionIdNumber } },
			{
				$lookup: {
					from: MONGO_CONSTANTS.COLLECTIONS.ORDERS,
					localField: MONGO_CONSTANTS.ORDERS.ORDER_ID,
					foreignField: MONGO_CONSTANTS.ORDERS.ORDER_ID,
					as: MONGO_CONSTANTS.AS.ORDER_DETAILS,
				},
			},
			{ $unwind: getDollarSigned(MONGO_CONSTANTS.AS.ORDER_DETAILS) },
			{
				$lookup: {
					from: MONGO_CONSTANTS.COLLECTIONS.ORDER_HISTORIES,
					localField: MONGO_CONSTANTS.ORDERS.ORDER_DETAILS_STATUS_ID,
		...
		if(transactionObject.length === 1) {
			ordersObj = setUserOrderFromTransactionObject(ordersObj, transactionObject[0], userDetails);
		} else {
			ordersObj = [];
		}
	} else {
		// Get the Orders list from DB
		 const userOrderList = await OrderSchema.aggregate([
			{ $match: { employeeId } },
			{ $sort: { createdAt: 1 } },
			{
				$lookup: {
...
exports.updateCart = async (req: Request, res: Response) => {
	...
	try {
			...
			for (const product of productList) {
				...
				if (cartProduct && product.quantity === 0) {
					...
					await CartSchema.create({
						...
						rewardPointsWhenAddedtoCart: product.productInfo.rewardPoints,
					});
				}
			}
		...
		logger.error("Error in User Service - Update Cart " + JSON.stringify(error?.message));
		const errorObject = {
...
exports.getCart = async (req: Request, res: Response) => {
	...
	try {
		const { employeeId } = res.locals.employeeinfo;
		const cartItems: Array<CartItems> = await CartSchema.aggregate([
			{ $match: { employeeId } },
			{
				$lookup: {
				...
				$replaceRoot: {
					newRoot: {
						$mergeObjects: [
							...
							{
								rewardPointsWhenAddedtoCart: "$$ROOT.rewardPointsWhenAddedtoCart",
							}
						],
					},
				},
		...
		// Check which orders belongs to the given user and push it into an Response Object
		const cartItemsResponse: CartItemsResponse = {cartItems: []} as CartItemsResponse;
		for(const cartItem of cartItems) {
			if(cartItem.rewardPoints === cartItem.rewardPointsWhenAddedtoCart && cartItem.isActive === true) {
				cartItemsResponse.cartItems.push({
		...
		logger.error("Error in User Service - Get Cart" + JSON.stringify(error?.message));
		sendResponse(
			statusMessages.SERVER_ERROR,
			statusCodes.INTERNAL_SERVER_ERROR,
...
const getCOItemsStructured = (COItems: any[]): COItem[] => {
	...
	structuredCOItems = COItems?.map((COItem: any) => {
		const { productId, title, rewardPoints, isCustomisable, productImgURL, isActive } =
			COItem.productDetails;
		let isError = false;
		let errorMessage = "";

		switch(true) {
			// If the product is 'Deactivated', then send error message accordingly
			case (isActive === false): 
			isError = true;
			errorMessage = userCheckoutErrorMessages.PRODUCT_HAS_BEEN_REMOVED;
			break;

			// If the product's cost has been changed, then send error message accordingly
			case (COItem.rewardPointsWhenAddedtoCart !== rewardPoints): 
			isError = true;
			errorMessage = userCheckoutErrorMessages.COST_CHANGED;
			break;
...
export const claimUserReward = async (req: Request, res: Response) => {
				...
				await RewardModel.updateOne(
					{ encryptedCouponCode: dbSecretCode },
					{
						$set: {
							...
							rewardType: REWARD_TYPES.REDEEMED,
						},
					},
				);
				sendResponse(
	...
	logger.info("Exiting - User Service - Claim User Points");
		...
		logger.error(
			"Error in User Service - Claim User Reward:" + JSON.stringify(error?.message),
		);
		sendResponse(
			statusMessages.ERROR,
			statusCodes.INTERNAL_SERVER_ERROR,
...
export const expireUserPoints = async () => {
	try {
		...
		logger.error("expireUserPoints :" + JSON.stringify(error?.message));
...
export const expireAllCoupons = async () => {
	try {
		...
		logger.error("expireAllCoupons :" + JSON.stringify(error?.message));
		throw new Error(error);
	}
}