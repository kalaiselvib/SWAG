...
import { couponEmail, rewardMail } from "../constants/message.constants";
	...
	try {
		...
		if (typeof reward.rewardPoints !== "number") {
			try {
				...
				const parsedRewardPoints = parseIntegerValue(reward.rewardPoints);
				if (
					typeof parsedRewardPoints !== "number" ||
					isNaN(parsedRewardPoints)
				) {
					throw new Error(logMessages.pointsInvalid);
				}
				responseObject.rewardPoints = parsedRewardPoints;
			} catch (e) {
				throw new Error(logMessages.pointsInvalid);
			}
		}
...
const validateRewards = async (
	...
	for (const reward of rewards) {
		try {
			...
			reward.date = moment(Date.now()).format("DD MMM YYYY");
...
const XLSkeletonCheck = (actualKeys: string[], expectedKeys: string[]): boolean => {
	return expectedKeys.every(
		(expectedKey) =>
			actualKeys.includes(expectedKey) ||
			actualKeys.includes(expectedKey.toLowerCase()),
	);
};

/**
 * @description Converts the user-understandable reward keys to camelCase so it's accessible by dot notation
 * @param rewards any[]
 * @returns rewards any[]
 */
const convertRewards = (rewards: any[]): any[] => {
	const properRewards = rewards.map((reward) => {
		const properReward: Record<any, any> = {};
		for (const [key, properKey] of Object.entries(rewardInputKeysMap)) {
			properReward[properKey] = reward[key] || reward[key.toLowerCase()];
		}
		return properReward;
	});
	return properRewards;
};
...
export const uploadRewards: (
	...
	if (!XLSkeletonCheck(actualRewardKeys, expectedRewardKeys)) {
		sendResponse(
	...
	const properRewards = convertRewards(rewards);
	properRewards.forEach((reward) => {
		reward.addedBy = employeeId;
	});
	// Validating the rewards and setting the response
	const { responseRewards, failureStatus } = await validateRewards(properRewards);
	failureStatus
		? sendResponse(
...
export const sendRewardEmail = async (reward: Reward): Promise<void> => {
	try {
		...
		if (rewardee?.email && reward?.transaction) {
			await sendEmail({
				toAddress: rewardee?.email,
				...rewardMail(reward, rewardee, reward.transaction),
			});
		} else {
			throw new Error("Rewardee email not available to send mail.");
		}
		logger.info("sent Reward Email to user");
	} catch (e: any) {
		logger.error(
			"Error while creating user reward email" + JSON.stringify(e?.message),
		);
	}
};
...
// 		const couponEmailContent = await couponEmail(reward, rewardee);
...
// 					},
// 					{
// 						filename: "cdw-logo.png",
// 						path: path.join(__dirname, "../images/cdw-logo.png"),
// 						cid: "logo",
// 					},
// 				],
// 			);
 ...
 */
export const createTransaction = async (reward: Reward): Promise<Transaction> => {
	let description;
...
const createOrGetRewardCategory = async (
	...
	rewardCategoryObject: RewardCategories;
...
export const createRewards: (
	...
	for (const reward of rewards) {
		...
		try {
			...
			const transaction: any = await createTransaction(parsedReward);
			parsedReward.transactionId = transaction?._id;
			parsedReward.encryptedCouponCode = null;
			parsedReward.isCouponExpired = true;
			parsedReward.rewardType = REWARD_TYPES.ADDED;
			// Coupon Generation
			...
			savedReward.transaction = transaction;
			...
			if (parsedReward.transactionId) {
				await TransactionModel.findByIdAndDelete(parsedReward.transactionId);
			}
	...
	await sendRewardHREmail(savedRewards, employeeId);
...
const formatSavedRewards = async (
	savedRewards: Reward[],
	isCoupon: boolean,
): Promise<Reward[]> => {
	const rewards: any[] = [];
...
/**
 * @description validates couponRequests
 * @param couponRequest
 * @returns boolean
 */
...
export const generateCoupons = async (
	...
	if (!hr || !hrEmailAddress) {
		sendResponse(
			statusMessages.BAD_REQUEST,
			statusCodes.BAD_REQUEST,
			errorMessages.INVALID_REQUESTOR,
			res,
		);
		return;
	}
	...
	// globals
	const savedCoupons: Reward[] = [];
	if (!couponRequests?.length) {
		sendEmptyRequestResponse(res);
		return;
	}
	let isSuccess = true;

	// iterating and creating coupons
	for (const couponRequest of couponRequests) {
		const { rewardCategory, rewardPoints, quantity } = couponRequest;
		if (isInvalidCoupon(rewardCategory, rewardPoints, quantity)) {
			logger.info(
				"Invalid Coupon:" +
					JSON.stringify({ rewardCategory, rewardPoints, quantity }),
			);
			isSuccess = false;
			continue;
		}
		const rewardCategoryRes = await createOrGetRewardCategory(rewardCategory);
		for (let quantityCount = 0; quantityCount < quantity; quantityCount++) {
			try {
				// generating coupon
				const { encryptedCouponCode, secretCode, couponCode } =
					generateCoupon(rewardCategory);

				// saving coupon object
				const savedCoupon = await RewardModel.create({
					encryptedCouponCode,
					isCouponExpired: false,
					rewardType: REWARD_TYPES.COUPON,
					rewardCategory: rewardCategoryRes?.rewardCategoryObject,
					description: COUPON_DESCRIPTION,
					rewardPoints,
					addedBy: hr,
				});
				// making coupon ready for email
				savedCoupon.couponCode = couponCode;
				savedCoupon.secretCode = secretCode;
				savedCoupons.push(savedCoupon);
				logger.info("Coupon successfully saved in db.");
			} catch (e: any) {
				logger.error(
					"error while generating coupon" + JSON.stringify(e?.message),
				);
				isSuccess = false;
			}
		}
	...
	try {
		if (savedCoupons?.length > 0) {
			await sendCouponHREmail(savedCoupons, hr);
			logger.info("Coupon emails sent to HR");
		}
	} catch (e: any) {
		logger.error(
			"Error while sending hr email for coupons generated:" +
...
/**
 * @description converts camelCased to spaced column names suitable for the XL email
 * @param rewards camelCased
 * @returns rewards spaced
 */
const formatDataForXL = (data: any[], map: Record<string, string>): any[] => {
	const properItems = data.map((item) => {
		const properItem: Record<any, any> = {};
		for (const [key, properKey] of Object.entries(map)) {
			properItem[properKey] = item[key];
		}
		return properItem;
	});
	return properItems;
 ...
 * @returns void
 */
const sendCouponHREmail = async (
	savedRewards: Reward[],
	hr: UserSchemaType,
): Promise<void> => {
	const hrEmailAddress = hr?.email;
	if (hrEmailAddress) {
		const formattedRewards: Reward[] = await formatSavedRewards(
			savedRewards,
			true,
		);
		const totalCouponsCreated = formattedRewards?.length;
		const pdfPath = await createMultiPagePDFwithData(formattedRewards);
		for (const formattedReward of formattedRewards) {
			delete formattedReward?.secretCode;
		}
		const xlPath = createXLFromData(
			formatDataForXL(formattedRewards, couponOutputKeysMap),
			PROMO_TYPES.COUPON,
		);
		...
		if (xlPath && pdfPath) {
			await sendEmail(
				{
					...COUPONS_EMAIL_CONTENT(hr, totalCouponsCreated),
				},
				[
					{
						path: xlPath,
...
/**
 * @description sends HR email
 * @param rewards
 * @returns void
 */
const sendRewardHREmail = async (
	savedRewards: Reward[],
	employeeId: number,
): Promise<void> => {
	const hr: UserSchemaType | null = await UserModel.findOne({ employeeId });
	...
	if (hrEmailAddress) {
		...
		const xlPath = createXLFromData(
			formatDataForXL(formattedRewards, rewardOutputKeysMap),
			PROMO_TYPES.REWARD,
		);
		// const protectedPdfPath = passwordProtectPDF(pdfPath, hr, true);
		if (xlPath) {
			await sendEmail(
				{
					...REWARDS_EMAIL_CONTENT(hr, totalRewardsCreated),
				},
				[
					{
						path: xlPath,
...
const mapRewardDaosToRewardDtos = async (
	...
	for (const rewardDAO of rewardDAOs) {
		...
		rewardDTOs.push({
			...
			rewardType: rewardDAO.rewardType,
		});
	}
 ...
 * @returns Response
 */
const generateCoupon: (rewardCategory: string) => CouponDetails = (
	rewardCategory: string,
) => {
	try {
		if (rewardCategory?.length < 3) {
			rewardCategory = "zzz";
		}
		const couponCode =
			rewardCategory.trim().slice(0, 3).toLowerCase() +
			"-" +
			crypto.randomBytes(6).toString("hex"); // couponCode for the user & hr
