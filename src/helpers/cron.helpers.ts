...
// logger.info(
// 	"expirationCronExpression, backupCronExpression, isExpirationJobEnabled, isDBBackupJobEnabled" +
// 		JSON.stringify(typeof expirationCronExpression) +
// 		JSON.stringify(expirationCronExpression) +
// 		JSON.stringify(typeof backupCronExpression) +
// 		JSON.stringify(backupCronExpression) +
// 		JSON.stringify(typeof isExpirationJobEnabled) +
// 		JSON.stringify(isExpirationJobEnabled) +
// 		JSON.stringify(typeof isDBBackupJobEnabled) +
// 		JSON.stringify(isDBBackupJobEnabled),
// );
const expirationtask = cron.schedule(expirationCronExpression, async() => {
    if(isExpirationJobEnabled === "true") {
        ...
        try {
            ...
            logger.error(
				"CRON Expiration Job Error:" + JSON.stringify(error?.message),
			);
        }
    }
},
...
const backupTask = cron.schedule(backupCronExpression, async() => {
    if(isDBBackupJobEnabled === "true") {
        ...
        try {
            ...
            logger.error(
				"CRON Backup Job : Error -" + JSON.stringify(error?.message),
			);
        }
    }
},
...
export const checkForJobsPastDueDates = async () => {
    try {
        ...
        logger.error("CRON : Jobs Past Due Date Error - " + JSON.stringify(error?.message));
    }
};