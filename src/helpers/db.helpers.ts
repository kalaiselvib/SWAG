import { UserInfo, type User } from "../types/user.type";
...
const ProductLogSchema = require("../schemas/productLog.schema");
/**
...
export const updateOrderStatus = async (
	...
	try {
		...
		const productName = order[0]?.productDetails?.title;
		const userEmail = order[0]?.userDetails[0]?.email;
		const quantity = order[0]?.quantity;
		const rewardPoints = order[0]?.productDetails?.rewardPoints;
		const productImgURL = order[0]?.productDetails?.productImgURL;
		...
		const userPoints = await PointSchema.findOne({ employeeId: userId });
		const statusEmailChange = await statusUpdateMail({
			userName,
...
export const createTransaction = async (
	...
	isCredited: boolean,
	amount: number,
	orderId: number = 0 , // Default value for 'orderId' (orderId = 0 denotes that there is no order associsted with this transaction)
) => {
...
export const getAllProductsFromDB = async (
	...
	try {
		let productsList = await ProductSchema.aggregate([
			{
				$lookup: {
					from: MONGO_CONSTANTS.COLLECTIONS.ORDERS,
					localField: MONGO_CONSTANTS.ORDERS.PRODUCT_ID,
					foreignField: MONGO_CONSTANTS.ORDERS.PRODUCT_DETAILED_ID,
		...
		// Remove InActive Products for Users
		if(employeeId) {
			productsList = productsList.filter((obj: Product) => obj.isActive === true);
		}
 ...
 * @param productObj { ProductModel }
 * @returns isUpdated { boolean }
 */
export const updateProductDetails = async (productObj: Product): Promise<boolean> => {
	let isUpdated = false;
	const updateObj = await ProductSchema.updateOne(
	...
	if(updateObj.matchedCount === 1 && updateObj.modifiedCount === 1) {
...
/**
 * Creates Edit Logs for Products
 * @returns void
 */
export const addProductEditLog = async (employeeInfo: UserInfo, isEditSuccess: boolean, productBeforeEdit: Product, productAfterEdit: Product, timeStamp: string): Promise<void> => {
	try {
		if(isEditSuccess) {
			const editLogs = await ProductLogSchema.create({
			...
			logger.info("addProductEditLogs - Log Created");
		}
	} catch (error: any) {
		logger.error("addProductEditLogs - " + JSON.stringify(error?.message));
	}
};