...
import { PROMO_TYPES } from "../constants/hr.constants";
import { COUPONS_FILE_NAME, REWARDS_FILE_NAME } from "../constants/helper.constants";
...
export const getJSONFromSheet = (file: Express.Multer.File): any[] | null => {
	...
	fs.unlink(file.path, (err) => {
		if (err) {
			logger.error("Error deleting file:" + JSON.stringify(err?.message));
		}
	});
...
export const createXLFromData = (
	...
	try {
		...
		let fileName;
		if (promoType === PROMO_TYPES.COUPON) {
			fileName = COUPONS_FILE_NAME;
		} else if (promoType === PROMO_TYPES.REWARD) {
			fileName = REWARDS_FILE_NAME;
		}
		const outputFile = `${fileName}${randomBytes(4).toString("hex")}.xlsx`;
