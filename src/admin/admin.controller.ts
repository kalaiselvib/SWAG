...
import { ProductDTO, DeleteProductDTO } from "../dto/product.dto";
...
exports.updateProductDetails = async (req: Request, res: Response) => {
	try {
		const { productId, title, rewardPoints, isCustomisable, productImgURL } = req.body;
		const errorMap = await classValidate(ProductDTO, productId, title, rewardPoints, isCustomisable, productImgURL);
		
		if (Object.keys(errorMap).length) {
			logger.error(
				"updateProductDetails - Class validator error",
			);
			sendResponse(
				statusMessages.BAD_REQUEST,
				statusCodes.BAD_REQUEST,
		...
		adminService.updateProductDetails(req, res);
	} catch (error: any) {
		logger.error("Server Error : ", JSON.stringify(error?.message));
	}
};
...
exports.deleteProduct = async (req: Request, res: Response) => {
	try {
		...
		const errorMap = await classValidate(DeleteProductDTO, productId);

		if (Object.keys(errorMap).length) {
			logger.error(
				"deleteProduct - Class validator error",
			);
			sendResponse(
				statusMessages.BAD_REQUEST,
				statusCodes.BAD_REQUEST,
...
exports.activateProduct = async (req: Request, res: Response) => {
	try {
		...
		adminService.activateProduct(req, res);
	} catch (error: any) {
		logger.error(JSON.stringify(error?.message));
		sendResponse(
			statusMessages.SERVER_ERROR,
